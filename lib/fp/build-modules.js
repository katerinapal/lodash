...
import { filejs_copy, filejs_globTemplate, filejs_write } from "../common/file";
import { mappingjs as mapping } from "../common/mapping";
import { utiljs_pitch } from "../common/util";
'use strict';

const templatePath = path.join(__dirname, 'template/modules');
const template = filejs_globTemplate(path.join(templatePath, '*.jst'));
...
function build(target) {
  ...
  const actions = modulePaths.map(modulePath => {
    ...
    return filejs_write(path.join(fpPath, moduleName + '.js'), getTemplate(moduleName));
  });

  actions.unshift(filejs_copy(path.join(__dirname, '../../fp'), fpPath));
  actions.push(filejs_write(path.join(fpPath, '_falseOptions.js'), template._falseOptions()));
  actions.push(filejs_write(path.join(fpPath, '_util.js'), template._util()));
  actions.push(filejs_write(path.join(target, 'fp.js'), template.fp()));
  actions.push(filejs_write(path.join(fpPath, 'convert.js'), template.convert()));

  async.series(actions, utiljs_pitch);
}
