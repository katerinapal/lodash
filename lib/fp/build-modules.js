...
import { file_copy, file_globTemplate, file_write } from "../common/file";
...
const template = file_globTemplate(path.join(templatePath, '*.jst'));
...
function build(target) {
  ...
  const actions = modulePaths.map(modulePath => {
    ...
    return file_write(path.join(fpPath, moduleName + '.js'), getTemplate(moduleName));
  });

  actions.unshift(file_copy(path.join(__dirname, '../../fp'), fpPath));
  actions.push(file_write(path.join(fpPath, '_falseOptions.js'), template._falseOptions()));
  actions.push(file_write(path.join(fpPath, '_util.js'), template._util()));
  actions.push(file_write(path.join(target, 'fp.js'), template.fp()));
  actions.push(file_write(path.join(fpPath, 'convert.js'), template.convert()));

  async.series(actions, util_pitch);
}
