...
var template = (0, _file.globTemplate)(_path2.default.join(templatePath, '*.jst'));

var aryMethods = _lodash2.default.union(mapping.aryMethod[1], mapping.aryMethod[2], mapping.aryMethod[3], mapping.aryMethod[4]);
...
function isAlias(name) {
  return _lodash2.default.has(mapping.aliasToReal, name);
}
...
function getTemplate(moduleName) {
  var data = {
    'name': _lodash2.default.get(mapping.aliasToReal, moduleName, moduleName),
    'mapping': mapping
  };
...
function build(target) {
  ...
  _lodash2.default.each([mapping.aliasToReal, mapping.remap], function (data) {
  ...
  var actions = modulePaths.map(function (modulePath) {
    ...
    return (0, _file.write)(_path2.default.join(fpPath, moduleName + '.js'), getTemplate(moduleName));
  });

  actions.unshift((0, _file.copy)(_path2.default.join(__dirname, '../../fp'), fpPath));
  actions.push((0, _file.write)(_path2.default.join(fpPath, '_falseOptions.js'), template._falseOptions()));
  actions.push((0, _file.write)(_path2.default.join(fpPath, '_util.js'), template._util()));
  actions.push((0, _file.write)(_path2.default.join(target, 'fp.js'), template.fp()));
  actions.push((0, _file.write)(_path2.default.join(fpPath, 'convert.js'), template.convert()));

  _async2.default.series(actions, _util.pitch);
}
