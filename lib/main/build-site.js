...
import ext_markymarkdown_marky from "marky-markdown";
import ext_path_path from "path";
import { utiljs as commonutil_utiljsjs } from "../common/util";
'use strict';

const basePath = ext_path_path.join(__dirname, '..', '..');
const docPath = ext_path_path.join(basePath, 'doc');
const readmePath = ext_path_path.join(docPath, 'README.md');
...
const exts = ext_lodash__.keys(highlights);
...
function tidyHighlights($) {
  $('.highlight').each(function() {
    ...
    const ext = ext_lodash__(classes).intersection(exts).last();
    ...
    $parent.find('[class]').each(function() {
      ...
      const attr = ext_lodash__(classes).intersection(highlights[ext]).join(' ');
...
function build() {
  ...
  const $ = ext_cheerio_cheerio.load(ext_markymarkdown_marky(markdown, {
  ...
  ext_fs_fs.writeFile(ext_path_path.join(docPath, version + '.html'), html, commonutil_utiljsjs.pitch);
}
